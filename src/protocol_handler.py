...
def parse_integer_and_size(data, terminator_start):
...
def parse_bulk_string_and_size(length, remainder, terminator_start):
    try:
        length = int(length)
        if length == -1:
...
def parse_array_and_size(number_of_elements, remainder, terminator_start):
    try:
